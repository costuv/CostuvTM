...
// Handle form success after redirection from Web3Forms
function handleFormSuccess() {
  ...
  if (formContainer) {
    ...
    successMsg.innerHTML = `
      ...
      <p style="color: #ffffff; margin: 15px 0;">Thank you for your message. I will get back to you as soon as possible.</p>
      <button class="send-btn" id="reset-form">Send another message</button>
    `;
...
// Check for form success URL parameter on page load
document.addEventListener('DOMContentLoaded', function() {
  // Check if we have a form=success parameter in the URL
...
// Sidebar Navigation - No Active State
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Add click event listeners to sidebar icons for smooth scrolling only
  sidebarIcons.forEach(icon => {
  ...
  if (uptimeDaysElement) {
    const launchDate = new Date('May 15, 2025');
    const today = new Date();
    const diffTime = Math.abs(today - launchDate);
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    uptimeDaysElement.textContent = diffDays;
  }
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      // No need to prevent default - we want the form to submit to Web3Forms
      
      // We'll show a loading state for better UX
      ...
      // But we'll add a backup in case the redirect doesn't work
      setTimeout(() => {
        if (submitButton.textContent === '$ sending...') {
          // Show success message if it's been more than 5 seconds
    ...
    function showSuccessMessage(name) {
      ...
      successMsg.innerHTML = `
        <div class="terminal-line">Message sent successfully!</div>
        <p style="color: #ffffff; margin: 15px 0;">Thank you for your message, ${name}. I will get back to you as soon as possible.</p>
        <button class="send-btn" id="reset-form">Send another message</button>
      `;
...
// Debug helper function - uncomment to enable debug info
/*
function debugSections() {
  ...
  debugInfo.style.cssText = 'position: fixed; bottom: 10px; right: 10px; background: black; color: lime; padding: 10px; z-index: 9999; font-family: monospace; font-size: 12px;';
  
  function updateDebugInfo() {
    const scrollY = window.scrollY;
    let html = `<div>Scroll position: ${scrollY}px</div>`;
    ...
    sections.forEach((section, i) => {
      if (section) {
        ...
        const isActive = (scrollY >= top - 100 && scrollY < (top + height - 100));
        html += `<div style="color: ${isActive ? '#8EFF9F' : 'white'}">
          Section #${i+1}: ${id}, top: ${top}px, height: ${height}px, active: ${isActive}
        </div>`;
      }
    });
